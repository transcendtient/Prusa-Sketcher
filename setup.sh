...
if [[ "$_java" ]]; then
  ...
  if [[ "$version" > "1.7" ]]; then
    echo -e ${Green}"\tYour installed Java version is 1.7+. Not installing Java."${Off} | tee -a log.txt 
#else check the distro repo default version
  else         
    echo -e ${Red}"\tYour installed Java version is < 1.7."${Off} | tee -a log.txt 
...
#repo sufficient then install it
    if [[ "$_DEF_JAVA" > "1.7" ]]; then
      echo -e ${Green}"\tYour default Java version is $_DEF_JAVA. Installing default."${Off} | tee -a log.txt 
      sudo apt-get install default-jdk
#else try to install manually
    else         
      echo -e ${Red}"\tYour default Java version is < 1.7. Trying to install manually."${Off} | tee -a log.txt 
...
if [[ "$_openscad" ]]; then
  ...
  if [[ "$version" > "2013.06" ]]; then
    echo -e ${Green}"\tYour OpenSCAD version is 2013.06+."${Off} | tee -a log.txt 
#else check the distro repo default
  else         
    echo -e ${Red}"\tYour OpenSCAD version is < 2013.06."${Off} | tee -a log.txt 
    ...
    if [[ "$_DEF_OPENSCAD" > "2013.06" ]]; then
      echo -e ${Green}"\tYour default OpenSCAD version is $_DEF_OPENSCAD. Installing..."${Off} | tee -a log.txt 
  ...
  if [[ "$_DEF_OPENSCAD" > "2013.06" ]]; then
    ...
    echo -e ${Red}"\tYour default OpenSCAD version is < 2013.06. \n\tCompiling from source. This will take a long time!"${Off} | tee -a log.txt 
#if we already have it...    
    if [ -d "./openscad/" ]; then
	  echo -e ${Green}"\tOpenSCAD directory present. Will not git."${Off} | tee -a log.txt 
	else
    ...
    echo -e ${Green}"Getting dependencies."${Off} | tee -a log.txt 
...
#parse fields from the check dependencies script
    ...
    for (( i=1; i <= ${#isOK[@]}; i++ )); do
      if [[ ${isOK[i]}  == "NotOK" ]]; then
        echo -e ${Red}"OpenSCAD dependencies not satisfied by script. \nBuilding ${nameDep[$i]} from source. This will take a long time!"${Off} | tee -a log.txt 
...
#parse fields again
    ...
    for (( i=1; i <= ${#isOK[@]}; i++ )); do
      ...
      if [[ ${isOK[i]}  == "NotOK" ]]; then
        echo -e ${Red}"OpenSCAD dependencies unable to be satisfied by script. \nABORTING!!"${Off} | tee -a log.txt 
        exit
      fi
      echo -e ${Red}"Making! This will take a long time!"${Off} | tee -a log.txt 
      qmake
      make
      make install
...
if [[ "$_DEF_KERNEL" == "x86_64" ]]; then
    echo -e ${Green}"\tUnzipping x64 KISSlicer."${Off} | tee -a log.txt 
    unzip ./Dependencies/KISSlicer_Linux64_1_1_0.zip -d ./kiss/
elif [[ "$_DEF_KERNEL" == "i686" ]]; then
    echo -e ${Green}"\tUnzipping i686 KISSlicer."${Off} | tee -a log.txt 
    unzip ./Dependencies/KISSlicer_Linux32_1_1_0.zip -d ./kiss/
else         
    echo -e ${Red}"\tYour kernel is unidentified. \n\tPlease take a look at the KISSlicer downloads section."${Off} | tee -a log.txt 
...
echo -e ${Green}"\tInstalling Printrun dependencies."${Off} | tee -a log.txt 
...
if [ -d "./Printrun/" ]; then
	  echo -e ${Green}"\tPrintrun directory present. Will not git."${Off} | tee -a log.txt 
else
    echo -e ${Green}"\tPrintrun directory not present. Will git."${Off} | tee -a log.txt 
    git clone https://github.com/kliment/Printrun.git
fi

echo -e ${Green}"\tInstalling core dependencies."${Off} | tee -a log.txt 
sudo apt-get install gimp imagemagick pstoedit librxtx-java potrace

echo -e ${Green}"\tCompiling java programs."${Off} | tee -a log.txt 
javac -classpath .:/usr/share/java/RXTXcomm.jar ./core/java/*.java

#copy GIMP sccript
if [ -d "/usr/share/gimp/2.0/scripts/" ]; then
  echo -e ${Green}"\tCopying GIMP script! It's over!"${Off} | tee -a log.txt 
...
else echo -e ${Red}"\tThe GIMP plugin directory was not found! Copy ./Dependencies/do-pencil-drawing.scm to your plugin directory to finish installation!"${Off} | tee -a log.txt 
fi

echo -e "\n\nHere's a log of the script activity."
cat log.txt
